...
const expoVersion = expoDependenciesPackageJson.devDependencies['expo'];
...
function generateReactNativeApp() {
  ...
  try {
    ...
    const generateCommand = `-y create-expo-app@${createExpoAppVersion} -t expo-template-blank-typescript@${expoTemplateVersion} --no-install ${name} npm i expo@${expoVersion}`;
